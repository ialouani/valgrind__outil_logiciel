...
struct Cell * append(struct Cell *list, struct Cell *newCell){
  ...
  p=list;//remonter: depuis les champs de list passee en parametre.
  while(p->next != NULL) p=p->next;//p->next non initialisee.
  p->next = newCell;
  newCell -> prev = p;
  return list;
}
...
void print(struct Cell *l){
  if (l==NULL) {//l pas initialisee par l'appel (up dans gdb)de l->next.
    //=>remonter vers les prev comme avant pour controler les append/remove
    //au niveau des interactions des champs next avec les AUTRES..+++
    printf("\n"); 
    return;
  }
  printf("%d ",l->value);
