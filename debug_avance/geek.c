...
void printList(struct Cell *list){
  if(list->next==NULL){printf("%d ",list->value); printf("\n");return;}
  //appel en remontant a list qui d'apres gdb target remote=>p *list=>=>

  //next:NULL donc a priori ca vaut mieux de tester sur list->next
  //vu l'initialisation de debug(but<<) d'avant(--).
  else {
    printf("%d ",list->value);
